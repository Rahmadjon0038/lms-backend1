...
exports.getMonthlyPayments = async (req, res) => {
  ...
  try {
    ...
    const params = role === 'teacher' ? [selectedMonth, userId] : [selectedMonth];
 ...
 */
    ...
    if (status) {
      if (status === 'paid') {
        query += ` AND COALESCE(sp.paid_amount, 0) >= COALESCE(sp.required_amount, GREATEST(g.price - COALESCE(
          (
            SELECT SUM(
              CASE 
        ...
        query += ` AND COALESCE(sp.paid_amount, 0) > 0 AND COALESCE(sp.paid_amount, 0) < COALESCE(sp.required_amount, GREATEST(g.price - COALESCE(
          (
            SELECT SUM(
              CASE 
                WHEN sd.discount_type = 'percent' THEN (g.price * sd.discount_value / 100)
...
exports.makePayment = async (req, res) => {
  ...
  try {
    ...
    // Shu oy uchun chegirma (agar bor bo'lsa)
    ...
    `, [student_id, group_id, selectedMonth]);

    const discountAmount = parseFloat(discountRow.rows[0].total_discount || 0);
    const requiredAmountCalculated = Math.max(parseFloat(student.price) - discountAmount, 0);
    ...
    if (paymentRecord.rows.length === 0) {
      // Yangi yozuv yaratish (chegirma hisobga olingan)
      await pool.query(`
        INSERT INTO student_payments 
        (student_id, month, group_id, required_amount, paid_amount, created_by)
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [student_id, selectedMonth, group_id, requiredAmountCalculated, newPaidAmount, adminId]);
      ...
      // Agar required_amount noto'g'ri (chegirmasiz) bo'lsa, uni qayta hisoblaymiz
      const currentRequired = parseFloat(paymentRecord.rows[0].required_amount || 0);
      const needsRequiredUpdate = currentRequired === parseFloat(student.price) && discountAmount > 0;
      const nextRequiredAmount = needsRequiredUpdate ? requiredAmountCalculated : currentRequired || requiredAmountCalculated;

      await pool.query(`
        UPDATE student_payments 
        SET paid_amount = $1, last_payment_date = NOW(), updated_by = $2, required_amount = $6
        WHERE student_id = $3 AND month = $4 AND group_id = $5
      `, [totalPaid, adminId, student_id, selectedMonth, group_id, nextRequiredAmount]);
    }
...
/**
 * 6.1 MUAYYAN OY/GURUH UCHUN TALABA TO'LOV MA'LUMOTLARINI TOZALASH
 * Faqat admin: berilgan student_id, group_id va month bo'yicha payment_transactions va student_payments o'chiriladi
 */
exports.clearStudentPaymentsByMonth = async (req, res) => {
  const { student_id, group_id, month, confirm } = req.body;
  const { id: adminId, name: adminName, role } = req.user;
  ...
  try {
    ...
    if (!student_id || !group_id || !month || !confirm) {
      return res.status(400).json({
        success: false,
        message: 'student_id, group_id, month va confirm=true majburiy (month: YYYY-MM)'
      });
    }
    ...
    // Student va guruh mavjudligini tekshirish (student guruhdan chiqgan bo'lsa ham to'lov tarixi bor bo'lishi mumkin)
    const studentCheck = await pool.query(`
    ...
    if (studentCheck.rows.length === 0) {
      return res.status(404).json({
        success: false,
        message: 'Student yoki guruh topilmadi'
      });
    }

    const student = studentCheck.rows[0];

    // Hisob-kitob (o'chiriladigan yozuvlar soni va summasi)
    const statsQuery = await pool.query(`
      SELECT 
        (SELECT COUNT(*) FROM payment_transactions WHERE student_id = $1 AND group_id = $2 AND month = $3) as transactions_count,
        (SELECT COUNT(*) FROM student_payments WHERE student_id = $1 AND group_id = $2 AND month = $3) as payments_count,
        (SELECT COUNT(*) FROM student_discounts WHERE student_id = $1 AND group_id = $2 AND start_month = $3) as discounts_count,
        (SELECT COALESCE(SUM(amount), 0) FROM payment_transactions WHERE student_id = $1 AND group_id = $2 AND month = $3) as total_paid_tx,
        (SELECT COALESCE(SUM(paid_amount), 0) FROM student_payments WHERE student_id = $1 AND group_id = $2 AND month = $3) as total_paid_sp
    `, [student_id, group_id, month]);

    const stats = statsQuery.rows[0];

    // Agar hech narsa bo'lmasa, xabar
    if (parseInt(stats.transactions_count) === 0 && parseInt(stats.payments_count) === 0 && parseInt(stats.discounts_count) === 0) {
      return res.status(404).json({
        success: false,
        message: 'Ko\'rsatilgan oy va guruh uchun tozalashga ma\'lumot topilmadi'
      });
    }
    ...
    try {
      ...
      console.log(`🗑️  Admin ${adminName} (ID: ${adminId}) ${student.name} ${student.surname} (ID: ${student_id}) uchun ${month} oy, group ${group_id} to'lov yozuvlarini tozaladi`);

      res.json({
        success: true,
        message: 'Ko\'rsatilgan oy va guruh bo\'yicha to\'lov ma\'lumotlari tozalandi',
        data: {
          student_name: `${student.name} ${student.surname}`,
          group_id,
          month,
          cleared_data: {
            ...
            payments_records: parseInt(stats.payments_count),
            discount_records: parseInt(stats.discounts_count),
            total_paid_removed: Math.max(parseFloat(stats.total_paid_tx || 0), parseFloat(stats.total_paid_sp || 0))
          },
          cleared_by: adminName,
          cleared_at: new Date().toISOString()
        }
    ...
    console.error('Student to\'lov ma\'lumotlarini (oy/guruh) tozalashda xatolik:', error);
    res.status(500).json({
      success: false,
      message: 'To\'lov ma\'lumotlarini tozalab bo\'lmadi',
      error: error.message
...
exports.exportMonthlyPayments = async (req, res) => {
  ...
  try {
    ...
    // Tarixiy oylar uchun saqlangan required_amount'ni ustun qo'yamiz
    const requiredAmountExpr = 'COALESCE(sp.required_amount, GREATEST(g.price - COALESCE(sdc.total_discount_amount, 0), 0))';
    ...
    let query = `
      ...
      SELECT 
        ...
        -- To'lov ma'lumotlari
                ${requiredAmountExpr} as required_amount,
        COALESCE(sp.paid_amount, 0) as paid_amount,
        COALESCE(sdc.total_discount_amount, 0) as discount_amount,
        CASE 
                  WHEN COALESCE(sp.paid_amount, 0) >= ${requiredAmountExpr} THEN 'Toʻliq toʻlagan'
          WHEN COALESCE(sp.paid_amount, 0) > 0 THEN 'Qisman toʻlagan'
          ELSE 'Toʻlamagan'
        END as payment_status,
        
                (${requiredAmountExpr} - COALESCE(sp.paid_amount, 0)) as debt_amount,
        TO_CHAR(sp.last_payment_date AT TIME ZONE 'Asia/Tashkent', 'DD.MM.YYYY HH24:MI') as last_payment_date
    ...
    if (status) {
      const statusCondition = 
        status === 'paid' ? `COALESCE(sp.paid_amount, 0) >= ${requiredAmountExpr}` :
        status === 'partial' ? `COALESCE(sp.paid_amount, 0) > 0 AND COALESCE(sp.paid_amount, 0) < ${requiredAmountExpr}` :
...
exports.getMyPayments = async (req, res) => {
  ...
  try {
    ...
    // Chegirma va talab qilingan summa uchun umumiy ifodalar (tarixiy oylar buzilmasligi uchun sp.required_amount birinchi o'rinda)
    const discountSubquery = `
      SELECT SUM(
        CASE 
          WHEN sd.discount_type = 'percent' THEN (g.price * sd.discount_value / 100)
          WHEN sd.discount_type = 'amount' THEN sd.discount_value
    ...
    const requiredAmountExpr = `COALESCE(sp.required_amount, GREATEST(g.price - COALESCE((${discountSubquery}), 0), 0))`;
    ...
    const query = `
      SELECT 
        ...
        -- Chegirma miqdori
        COALESCE((${discountSubquery}), 0) as discount_amount,
        ...
        -- To'lov holati
        CASE 
          WHEN COALESCE(sp.paid_amount, 0) >= ${requiredAmountExpr} THEN 'paid'
          WHEN COALESCE(sp.paid_amount, 0) > 0 THEN 'partial'
          ELSE 'unpaid'
        END as payment_status,
        
        -- Qarz miqdori
        GREATEST(${requiredAmountExpr} - COALESCE(sp.paid_amount, 0), 0) as remaining_amount,
...
exports.getMyPaymentHistory = async (req, res) => {
  const { group_id, limit = 10, student_id, month } = req.query;
  const { role, id: userId } = req.user;
  ...
  try {
    ...
    const params = [targetStudentId, selectedMonth];
    let paramCount = 2;
